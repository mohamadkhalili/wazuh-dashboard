...
if [[ "${WAZUH_SKIP_BUNDLE_BUILD:-0}" != 1 ]]; then
  ...
  printf '[wazuh-source] Building changed Wazuh browser bundles from current source...\n'
  su node -s /bin/bash -c \
    'cd /workspace/wazuh-dashboard && node docker/build-wazuh-source-bundles.js wazuh wazuhCore wazuhCheckUpdates securityDashboards wazuhRtl wazuhFarsi'
fi
