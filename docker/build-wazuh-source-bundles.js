...
async function main() {
  ...
  const config = OptimizerConfig.create({
    ...
    includeCoreBundle: requestedBundleIds.includes('core'),
    cache: true,
    watch: false,
    examples: false,
    maxWorkerCount: 1,
  ...
  const optimizerCacheKey = 'wazuh-4.14.6-source-v2';
  const outputFileFor = (bundle) =>
    Path.join(
      bundle.outputDir,
      bundle.id === 'core' ? 'core.entry.js' : `${bundle.id}.plugin.js`
    );
  ...
  for (const bundle of bundlesToBuild) {
    const outputFile = outputFileFor(bundle);
